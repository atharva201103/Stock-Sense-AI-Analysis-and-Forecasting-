...
const CandleStick = (props: any) => {
  ...
  return (
    <g>
      {/* Wick line */}
      <line x1={x + halfWidth} y1={y.low} x2={x + halfWidth} y2={y.high} stroke={color} strokeWidth={1} />
      {/* Candle body */}
      <rect
        x={x}
        y={isRising ? y.close : y.open}
        width={width}
        height={Math.abs(y.open - y.close)}
        fill={color}
        stroke={color}
      />
    </g>
...
export function NiftyChart({ symbol, name, currentPrice, change, changePercent }: NiftyChartProps) {
          ...
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              {chartData.map((entry, index) => {
                // Calculate positions for candle rendering
                const xPos = index * (100 / chartData.length)
                const width = 0.5 * (100 / chartData.length)

                // Calculate y positions based on the chart's y-scale
                const yScale = (max: number, min: number) => {
                  const range = max - min
                  return (val: number) => {
                    const ratio = (val - min) / range
                    return 400 * (1 - ratio) // 400 is the chart height
                  }
                }

                // Get min and max values for scaling
                const allValues = chartData.flatMap((d) => [d.high, d.low, d.open, d.close])
                const min = Math.min(...allValues)
                const max = Math.max(...allValues)
                const getY = yScale(max, min)
                ...
                return (
                  <CandleStick
                    ...
                    width={width}
                    y={{
                      open: getY(entry.open),
                      close: getY(entry.close),
                      high: getY(entry.high),
                      low: getY(entry.low),
                    }}
                    low={entry.low}
                    high={entry.high}
  ...
  return (
    <Card className="col-span-3">
      ...
      <CardContent>
        ...
        {/* Remove buy option for Nifty 50 */}
        {symbol !== "NIFTY 50" && (
          <div className="mt-4">
            <Button
              className="w-full"
              onClick={() => (window.location.href = `/dashboard/trade?symbol=${symbol}&action=buy`)}
            >
              Trade {symbol}
            </Button>
          </div>
